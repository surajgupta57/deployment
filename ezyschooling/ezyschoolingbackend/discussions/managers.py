    ...
    def like_comment_count(self):
        return self.annotate(
            ...
            comment_count=models.Count(
                "discussion_comments",
                distinct=True,
                filter=models.Q(discussion_comments__parent_comment__isnull=True),
            ),
        )

    def get_foreignkey_select_related(self):
        return self.select_related("board", "sub_category", "parent", "expert",)
...
class DiscussionCommentQuerySet(models.QuerySet):
    """Personalized queryset created to improve model usability"""
    ...
    def like_comment_count(self):
        return self.annotate(
            ...
            children_comment_count=models.Count(
                "user_discussion_comment_childrens", distinct=True
            ),
        )
